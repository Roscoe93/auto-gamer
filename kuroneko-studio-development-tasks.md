...
**4.1 截图采集层 (Capture)**
- [x] 评估并接入跨平台高速截图方案（如 `mss` 或 macOS Quartz / Windows BitBlt），确保单帧捕获耗时 < 50ms。
- [x] 实现基于目标窗口句柄的相对区域裁剪，剔除无用的桌面背景。

**4.2 视觉识别与组装层 (Recognition & Overlay)**
- [x] 接入 OpenCV，实现基础的 `matchTemplate`（模板匹配）。
- [x] 引入 `RapidOCR (ONNX)` 实现跨平台文本检测。
- [x] 提取匹配结果，生成结构化的 Overlay 元数据：包含坐标 `(x, y, w, h)`、标签名和置信度 (Score)。

**4.3 传输与节流层 (Bridge & Throttle)**
- [x] 定义 `PreviewFrame` 数据协议，打包图像流（本地文件路径 / Base64）与 Overlay 数据。
- [x] 实现 WebSocket 帧率节流 (Throttle)：控制预览推送最高不超过 5~10 FPS，避免高频 I/O 拖死 UI 渲染线程。
...
- [x] 实现结构化日志模型 (带 `event_type` 和关联 run)，并通过 WebSocket 实时推送。
- [x] 实现前端日志控制台：按级别、事件类型过滤与自动滚动。
