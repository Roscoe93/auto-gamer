...
为了保证高频图像流不会拖死前端 UI，以及识别坐标的精准映射，该模块按“截图 -> 识别 -> 传输 -> 展示”的链路进行如下细化设计：

**1. 截图采集层 (Capture)**
在会话 (Session) 准备阶段，用户从前端下拉列表中选择目标游戏窗口，系统将该窗口句柄（Handle ID，如 `win_12345`）存入全局上下文。
截图采集服务在每次捕获前，会通过 OS Adapter 调用 `get_window_rect(window_id)` 动态获取该句柄在屏幕上的最新物理边界 `(x, y, w, h)`。
随后，使用 `mss` 库直接根据这组动态坐标对屏幕进行精确裁剪 (Crop)。如果目标窗口被遮挡或无法获取坐标，则调用 `bring_to_front(window_id)` 强制其置顶。必须保证单帧捕获延迟在 `< 50ms`，且即使游戏窗口被拖动，捕获框也能实时跟随。
...
**4. 前端预览渲染层 (Preview UI)**
前端渲染组件分为两层：底层为 `<img>` 标签展示 Base64/本地图像，上层覆盖一个等大的绝对定位 `<div>` 或 `<svg>` 用于渲染 Bounding Box。必须通过 CSS 确保无论外层容器如何缩放，上层的坐标映射框始终与底图保持严格的等比缩放对齐 (Aspect Ratio Preserved)。

**5. 状态推断卡片 (Inference Card)**
在预览区侧边实时展示“Current Inference（当前看到了什么）”和“Next Action（准备做什么动作）”，帮助开发者理解状态机卡在哪里。
...
- 单脚本与参数 schema 动态表单
- 实时结构化日志与可视化识别预览叠层
- 硬件级跨平台输入模拟 (Windows/macOS)
- 全局安全中断热键 (Kill Switch)
- run 历史与错误产物打包导出
